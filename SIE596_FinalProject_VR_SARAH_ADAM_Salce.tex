...
\maketitle
\begin{abstract}
%Stochastic and Minibatch Gradient Descent based optimization algorithms have become increasingly popular in the past decade driven by the need for better computational efficiency for machine learning applications handling large datasets.  In this paper we examine two such algorithms, ADAM and SARAH, and their applications in convex optimization.  
%ADAM (ADAptive Moment estimation) utilizes minibatch gradients as well as RMSProp to reduce the effects of variance in its steps, as well as Momentum to adaptively step toward the optimum of a convex objective function. The convergence rate of ADAM is comparable to the best possible for optimization of convex objectives.  ***numerical exp result***
%SARAH is a modification of the Stochastic Variance Reduced Gradient (SVRG) that utilizes recursive gradient information rather than using only the information of the initialized full gradient for variance reduction.  It offers comparable but generally improved convergence over SVRG, and maintains the computational advantages held by SVRG over methods like SAG and SAGA.  ***numerical exp result***
